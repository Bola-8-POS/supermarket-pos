...
- **Checkout keypad** (`features/checkout-keypad`, composed into `CheckoutPanel`) — tap-only on-screen numeric keypad; `× Qty` arms a multiplier applied to the next product add, `Add (PLU)` adds an exact-barcode match through the same risky-add confirm gate as a grid tile tap; visibility toggle persisted per terminal in localStorage (`pos.keypad_visible`).
