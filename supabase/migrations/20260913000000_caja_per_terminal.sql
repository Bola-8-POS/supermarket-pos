-- Caja per terminal: one open register session per terminal instead of one
-- per database. Two changes:
--   1. caja_sessions gains terminal_id; the DB-wide "one open caja" unique
--      index becomes per-terminal; caja_open now persists it (previously
--      only forwarded to record_audit's terminal_id column).
--   2. process_direct_sale_atomic gains a 20th trailing parameter
--      p_terminal_id text DEFAULT NULL and rejects a caja belonging to a
--      different terminal with code CAJA_CLOSED. Body copied VERBATIM from
--      20260912000002_platform_tenders_rappi_uber_eats.sql (lines 623-973,
--      the latest live definition) except for that one guard + the new
--      trailing parameter.
BEGIN;
...
-- -----------------------------------------------------------------------------
-- caja_open — signature unchanged (numeric, uuid, text) from
-- 20260703000003_caja_open_prep_batch_rpcs.sql; body now persists
-- p_terminal_id onto the new column instead of only forwarding it to
-- record_audit.
-- -----------------------------------------------------------------------------
...
-- 20260912000002_platform_tenders_rappi_uber_eats.sql (lines 623-973), with
-- exactly two changes:
--   1. new trailing parameter p_terminal_id text DEFAULT NULL
--   2. the existing caja-open guard additionally captures the caja's
--      terminal_id and rejects a mismatch with code CAJA_CLOSED
-- Because the signature changes (19 args -> 20 args), the 19-arg overload is
-- dropped first so PostgREST never sees two overloads of the same name.
-- -----------------------------------------------------------------------------
